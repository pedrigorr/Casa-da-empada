...
export default function DriverDashboard({user}) {
  ...
  const [confModal,setConfModal]= useState(null); // order para confirmar entrega
  ...
  useEffect(()=>{
    let prevSaiu = new Set();
    const unsub = onSnapshot(collection(db,"orders"),snap=>{
      const all = snap.docs.map(d=>({...d.data(),_docId:d.id}));
      // Notifica se novo pedido "saiu"
      const nowSaiu = new Set(all.filter(o=>o.status==="saiu").map(o=>o.id));
      if(prevSaiu.size>0) {
