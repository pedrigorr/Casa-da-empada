...
import { Toast, Spinner, Modal, StatCard, Card, fieldStyle, Logo, WaButton } from "./UI.jsx";
...
export default function DriverDashboard({ user }) {
  ...
  useEffect(() => {
    let prevMine = new Set();
    const unsub = onSnapshot(collection(db, "orders"), snap => {
      const all = snap.docs.map(d => ({ ...d.data(), _docId: d.id }));
      const mine = all.filter(o => o.entregadorId === user.uid && o.status === "saiu");
      const mineIds = new Set(mine.map(o => o.id));
      if (prevMine.size > 0) {
        for (const id of mineIds) {
          if (!prevMine.has(id)) { playBeep(); showToast("🛵 Novo pedido atribuído a você!", "warn"); break; }
        }
      }
      prevMine = mineIds;
  ...
  // Só pedidos do entregador logado
  const myDeliveries = orders.filter(o => o.entregadorId === user.uid && o.status === "saiu");
  ...
  const myDone     = orders.filter(o => o.entregadorId === user.uid && (o.status === "entregue" || o.status === "retirado") && new Date(o.createdAt) >= rangeStart(period));
  ...
  async function handleDelivered(order) {
    ...
    if (order._docId) {
      await updateDoc(doc(db, "orders", order._docId), {
        status:         "entregue",
    ...
    <div style={{ minHeight: "100vh", background: C.cream }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; -webkit-tap-highlight-color: transparent; }
        input:focus, textarea:focus { border-color: ${C.terracotta} !important; box-shadow: 0 0 0 3px rgba(166,75,42,.15); outline: none; }
      ...
      <header style={{ background: "linear-gradient(135deg," + C.espresso + "," + C.brown + ")", padding: "12px 16px 0", boxShadow: "0 4px 24px rgba(0,0,0,0.3)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 9, color: C.sand, letterSpacing: 2, textTransform: "uppercase" }}>Painel do Entregador</div>
          </div>
          {myDeliveries.length > 0 && (
            <div style={{ background: C.terracotta, color: "#fff", borderRadius: 20, padding: "3px 12px", fontSize: 13, fontWeight: 700, fontFamily: "Georgia,serif", animation: "pulse 2s infinite" }}>
              {myDeliveries.length} para entregar
            </div>
          )}
      ...
      {!loading && tab === "entregas" && (
        <div style={{ padding: "14px", maxWidth: 480, margin: "0 auto", paddingBottom: 120 }}>
          {myDeliveries.length === 0 ? (
            ...
            <Card key={order.id} style={{ marginBottom: 12, border: "2px solid " + C.teal }}>
              <div style={{ padding: "14px" }}>
                ...
                <div style={{ background: C.cream, borderRadius: 8, padding: "8px 10px", marginBottom: 10, fontFamily: "Georgia,serif", fontSize: 11, color: C.espresso }}>
                  {(order.items || []).slice(0, 3).map((it, i) => <div key={i}>{it.qty}× {it.name}</div>)}
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 7, marginTop: 7 }}>
                  ...
                  <div style={{ display: "flex", gap: 6 }}>
                    {order.customerPhone && <WaButton phone={order.customerPhone} style={{ flex: 1, borderRadius: 11, height: 40, fontSize: 11 }} label="Cliente" />}
                    <WaButton phone={WHATSAPP_ADMIN} style={{ flex: 1, borderRadius: 11, height: 40, background: "#2563eb", fontSize: 11 }} label="Admin" />
                  </div>
                </div>
              </div>
            </Card>
        ...
        <Modal onClose={() => { setConfModal(null); setRecVal(""); }}>
          ...
          {confModal.payment === "dinheiro" && (
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontFamily: "Georgia,serif", fontSize: 11, fontWeight: 700, color: C.rust, letterSpacing: 1, marginBottom: 6 }}>VALOR RECEBIDO (R$)</div>
              <input value={recVal} onChange={e => setRecVal(e.target.value)} placeholder="Valor recebido"
                style={{ width: "100%", padding: "14px", borderRadius: 12, border: "1.5px solid " + C.parchment, background: "#fff", fontFamily: "Georgia,serif", fontSize: 18, outline: "none", boxSizing: "border-box", textAlign: "center" }} />
              {recVal && (
                <div style={{ textAlign: "center", fontFamily: "Georgia,serif", fontSize: 14, color: "#16a34a", marginTop: 8, fontWeight: 700 }}>
      ...
      <div style={{ position: "fixed", bottom: 24, right: 16, display: "flex", flexDirection: "column", gap: 10, zIndex: 90 }}>
        <a href={"https://wa.me/" + WHATSAPP_ADMIN} target="_blank" rel="noreferrer"
          style={{ width: 52, height: 52, borderRadius: "50%", background: "#25D366", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24, boxShadow: "0 4px 16px rgba(37,211,102,.4)", textDecoration: "none" }}>📲</a>
      </div>
