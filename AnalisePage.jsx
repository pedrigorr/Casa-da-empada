...
export default function AnalisePage({ orders, clients, caixaHistory }) {
  ...
  const bairroMap = {};
    ...
    if (!o.address) return;
    const p = o.address.split(",");
    const b = (p[2] || p[1] || p[0] || "").trim();
  ...
  const Row = ({ left, right, sub, color = C.terracotta }) => (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "7px 0", borderBottom: "1px solid " + C.parchment }}>
      <div>
        <div style={{ fontFamily: "Georgia,serif", fontSize: 13, color: C.espresso }}>{left}</div>
        {sub && <div style={{ fontSize: 11, color: C.sand }}>{sub}</div>}
      </div>
  ...
  return (
    <div style={{ padding: "12px 14px", maxWidth: 640, margin: "0 auto", paddingBottom: 50 }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 14 }}>
        ...
        <StatCard emoji="🛵" label="Fretes"       value={fmt(totalFrete)} accent={C.teal} />
      </div>

      <Section title="🏆 Top 10 Clientes">
        {topClientes.length === 0
          ? <div style={{ color: C.sand, fontFamily: "Georgia,serif", fontSize: 13, textAlign: "center", padding: 12 }}>Sem dados</div>
          : topClientes.map((c, i) => (
      ...
      {topItems.length > 0 && (
        <Section title="🥧 Mais Vendidos">
          {topItems.map((it, i) => <Row key={it.name} left={medals[i] + " " + it.name} right={it.qty + " un"} sub={fmt(it.revenue)} />)}
        </Section>
      )}

      {lowItems.length > 0 && (
        <Section title="📉 Menos Vendidos">
          {lowItems.map(it => <Row key={it.name} left={it.name} right={it.qty + " un"} color={C.rust} />)}
        </Section>
      )}
      ...
      <Section title="💳 Formas de Pagamento">
        {payRanking.map((p, i) => (
          <Row key={p.id}
            left={(i === 0 ? "👑 " : i === payRanking.length - 1 ? "⚠️ " : "• ") + p.label}
            right={p.count + " uso" + (p.count !== 1 ? "s" : "")}
            color={i === 0 ? C.sage : i === payRanking.length - 1 ? C.rust : C.terracotta}
          />
        ))}
      </Section>

      {topBairros.length > 0 && (
        <Section title="📍 Bairros com Mais Pedidos">
          {topBairros.map(([b, n], i) => <Row key={b} left={medals[i] + " " + b} right={n + " pedido" + (n !== 1 ? "s" : "")} />)}
        </Section>
      )}

      {Object.keys(cidMap).length > 0 && (
        <Section title="🗺️ Por Cidade">
          {Object.entries(cidMap).sort((a, b) => b[1] - a[1]).map(([c, n]) => <Row key={c} left={c} right={n + " pedido" + (n !== 1 ? "s" : "")} />)}
        </Section>
      )}
