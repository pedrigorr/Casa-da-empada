...
function ClienteInput({value, onChange, clients}) {
  ...
  return (
    <div ref={ref} style={{position:"relative"}}>
      <input value={value} onChange={e=>{onChange("name",e.target.value);setOpen(true);}} onFocus={()=>setOpen(true)}
        placeholder="Nome do cliente" style={fieldStyle()}/>
      {open&&suggestions.length>0&&(
        <div style={{position:"absolute",top:"100%",left:0,right:0,zIndex:300,background:C.warmWhite,border:`2px solid ${C.terracotta}`,borderRadius:"0 0 12px 12px",boxShadow:`0 8px 24px ${C.shadow}`,overflow:"hidden"}}>
          {suggestions.map(c=>(
            <div key={c.name} onMouseDown={()=>{onChange("client",c);setOpen(false);}}
              style={{padding:"11px 14px",cursor:"pointer",borderBottom:`1px solid ${C.parchment}`,fontFamily:"Georgia,serif",fontSize:13,display:"flex",justifyContent:"space-between",alignItems:"center"}}
...
export default function NovoPedido({onSave, showToast, clients}) {
  ...
  const itemsTotal = Object.entries(cart).reduce((s,[id,q])=>{ const it=ALL_ITEMS.find(x=>x.id===Number(id)); return s+(it?it.price*q:0); },0);
  ...
  async function submit() {
    ...
    try {
      await onSave({
        customer:name, customerPhone:phone, address:addr, cidade, tipo, payment:pay,
      ...
      setCart({}); setName(""); setPhone(""); setAddr(""); setPayVal(""); setNote(""); setSearch(""); setSignal(""); setDelivDate(""); setDelivTime(""); setFreight("");
    } catch { showToast("Erro ao salvar!","error"); }
    setSaving(false);
  }

  const renderItems = items=>items.map(item=>(
    <div key={item.id} style={{display:"flex",alignItems:"center",gap:10,background:C.warmWhite,borderRadius:12,padding:"10px 13px",border:`1px solid ${C.parchment}`}}>
      <div style={{flex:1}}>
  ...
  return (
    <div style={{padding:"12px 14px",display:"flex",flexDirection:"column",gap:14,maxWidth:640,margin:"0 auto",paddingBottom:40}}>
      {/* Tipo */}
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:7}}>
        {[["delivery","🛵","Delivery"],["retirada","🏠","Retirada"],["encomenda","📅","Encomenda"]].map(([v,ic,l])=>(
          <button key={v} onClick={()=>setTipo(v)} style={{padding:"11px 4px",borderRadius:12,border:`2px solid ${tipo===v?C.terracotta:C.parchment}`,background:tipo===v?C.terracotta:C.warmWhite,color:tipo===v?"#fff":C.brown,fontFamily:"Georgia,serif",fontSize:12,cursor:"pointer",fontWeight:700,boxShadow:tipo===v?`0 3px 12px rgba(196,101,58,.3)`:"none"}}>
      ...
      <div>
        ...
        <input value={phone} onChange={e=>setPhone(e.target.value)} placeholder="WhatsApp do cliente (opcional)" style={fieldStyle({marginTop:8})}/>
      </div>
      ...
      <div>
        ...
        <div style={{display:"flex",gap:6}}>
          {CIDADES.map(c=>(
            <button key={c} onClick={()=>setCidadeOpt(c)} style={{flex:1,padding:"9px 4px",borderRadius:10,border:`2px solid ${cidadeOpt===c?C.teal:C.parchment}`,background:cidadeOpt===c?C.teal:C.warmWhite,color:cidadeOpt===c?"#fff":C.brown,fontFamily:"Georgia,serif",fontSize:12,cursor:"pointer",fontWeight:700}}>{c}</button>
          ))}
        </div>
        {cidadeOpt==="Outra"&&<input value={cidadeText} onChange={e=>setCidadeText(e.target.value)} placeholder="Digite a cidade..." style={fieldStyle({marginTop:8})}/>}
      </div>
      ...
      {(tipo==="delivery"||tipo==="encomenda")&&(
        <div>
          ...
          <input value={addr} onChange={e=>setAddr(e.target.value)} placeholder="Rua, número, bairro..." style={fieldStyle()}/>
        </div>
      )}
      ...
      {tipo==="encomenda"&&(
        <div style={{background:"#EDE7F6",borderRadius:14,padding:14,border:"1.5px solid #B39DDB"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:10}}>
            <div><Lbl>Data *</Lbl><input type="date" value={delivDate} onChange={e=>setDelivDate(e.target.value)} style={fieldStyle()}/></div>
            <div><Lbl>Horário</Lbl><input type="time" value={delivTime} onChange={e=>setDelivTime(e.target.value)} style={fieldStyle()}/></div>
          </div>
          <Lbl>Sinal Pago (R$)</Lbl>
          <input value={signal} onChange={e=>setSignal(e.target.value)} placeholder="0,00" style={fieldStyle()}/>
          {signalNum>0&&<div style={{display:"flex",justifyContent:"space-between",fontFamily:"Georgia,serif",fontSize:12,marginTop:6}}>
      ...
      {/* Frete */}
      <div>
        <Lbl>Frete / Comissão do Entregador (R$)</Lbl>
      ...
      <div>
        ...
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="🔍 Buscar produto..." style={fieldStyle()}/>
      </div>

      {filtered?(
        <div style={{display:"flex",flexDirection:"column",gap:6}}>{filtered.length===0?<div style={{textAlign:"center",color:C.sand,fontFamily:"Georgia,serif",padding:16}}>Nada encontrado</div>:renderItems(filtered)}</div>
      ...
      {cartItems.length>0&&(
        <div style={{background:C.parchment,borderRadius:14,padding:14,border:`1.5px solid ${C.sand}`}}>
          ...
          {freightNum>0&&<div style={{display:"flex",justifyContent:"space-between",fontSize:13,fontFamily:"Georgia,serif",color:C.teal,margin:"5px 0"}}>
      ...
      <div>
        ...
        <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
          {PAYMENT_METHODS.map(m=>(
            <button key={m.id} onClick={()=>setPay(m.id)} style={{padding:"9px 13px",borderRadius:10,border:`2px solid ${pay===m.id?C.terracotta:C.parchment}`,background:pay===m.id?C.terracotta:C.warmWhite,color:pay===m.id?"#fff":C.brown,fontFamily:"Georgia,serif",fontSize:12,cursor:"pointer",fontWeight:700}}>{m.label}</button>
          ))}
        </div>
        {pay==="dinheiro"&&<div style={{display:"flex",gap:8,alignItems:"center",marginTop:8}}>
          <input value={payVal} onChange={e=>setPayVal(e.target.value)} placeholder="Valor recebido (R$)" style={fieldStyle({flex:1})}/>
          {payVal&&<span style={{fontSize:13,color:"#16a34a",fontWeight:700,fontFamily:"Georgia,serif",whiteSpace:"nowrap"}}>Troco: {fmt(change)}</span>}
        </div>}
      </div>
      ...
      <div>
        <Lbl>Observações</Lbl>
        <textarea value={note} onChange={e=>setNote(e.target.value)} placeholder="Ex: sem cebola, sabores específicos..." rows={2} style={fieldStyle({resize:"none"})}/>
      </div>
      ...
      <div style={{background:"#f0fdf4",borderRadius:12,padding:"10px 14px",border:"1.5px solid #86efac",display:"flex",gap:8,alignItems:"center"}}>
        <span>📲</span>
        <span style={{fontFamily:"Georgia,serif",fontSize:12,color:"#16a34a"}}>O resumo será enviado ao WhatsApp da Casa da Empada ao confirmar.</span>
      </div>
