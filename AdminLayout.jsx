            ...
            <div style={{padding:16}}>
              ...
              <div style={{background:C.cream,borderRadius:12,padding:12,marginBottom:15}}>
                {order.items?.map((it,idx)=>(
                  <div key={idx} style={{display:\"flex\",justify:\"space-between\",fontSize:13,marginBottom:4,color:C.espresso}}>
                    <span><b>{it.q}x</b> {it.name}</span>
                    <span style={{fontWeight:700}}>{fmt(it.price * it.q)}</span>
                  </div>
                ))}
                <div style={{marginTop:8,paddingTop:8,borderTop:`1px solid ${C.parchment}`,display:\"flex\",justify:\"space-between\",fontWeight:800,color:C.terracotta}}>
                  <span>TOTAL</span>
...
export default function AdminLayout({user}) {
  ...
  const [drivers, setDrivers] = useState([]); // Novo estado para entregadores
  ...
  useEffect(() => {
    ...
    // ESCUTA ENTREGADORES (Filtra apenas quem tem role 'driver')
