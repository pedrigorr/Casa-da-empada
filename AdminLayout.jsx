...
import { C, WHATSAPP_ADMIN, fmt, fmtDt, isDone, ORDER_STATUS, PAYMENT_METHODS, MENU, ALL_ITEMS, CIDADES } from "../utils/constants.js";
import { Toast, Spinner, Modal, Pill, SBadge, Card, StatCard, fieldStyle, Lbl, BtnPrimary, WaButton, Logo } from "../components/UI.jsx";
  ...
  return (
    <header style={{background:`linear-gradient(135deg,${C.espresso},${C.brown})`,position:"sticky",top:0,zIndex:100,boxShadow:`0 4px 24px rgba(0,0,0,0.3)`}}>
      <div style={{display:"flex",alignItems:"center",padding:"10px 16px 6px",gap:10}}>
        ...
        <button onClick={()=>signOut(auth)} style={{background:"rgba(255,255,255,0.1)",border:"none",color:C.sand,padding:"6px 10px",borderRadius:8,cursor:"pointer",fontSize:11,fontFamily:"Georgia,serif"}}>
          Sair
        </button>
      </div>
      <nav style={{display:"flex",overflowX:"auto",gap:2,padding:"0 8px 6px"}}>
        {tabs.map(t=>(
          <button key={t.id} onClick={()=>setTab(t.id)} style={{
            ...
            background:tab===t.id?`rgba(196,101,58,0.9)`:"transparent",
...
// ─── PEDIDOS ────────────────────────────────────────────────────
function PedidosPage({orders, onStatus, onDelete, clients}) {
  ...
  const client = (name) => clients.find(c=>c.name?.toLowerCase()===name?.toLowerCase());
  ...
  return (
    <div style={{padding:"12px 14px",maxWidth:640,margin:"0 auto",paddingBottom:40}}>
      ...
      <div style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:8,marginBottom:12}}>
        {statusFilters.map(f=>{
          const cnt = f.id==="entregue" ? orders.filter(o=>isDone(o.status)).length : orders.filter(o=>o.status===f.id).length;
      ...
      <div style={{display:"flex",flexDirection:"column",gap:10}}>
        {visible.map(order=>{
          ...
          const cli = client(order.customer);
            ...
            <Card key={order.id} style={{border:`1.5px solid ${isExp?C.terracotta:C.parchment}`}}>
                ...
                <div style={{display:"flex",alignItems:"flex-start",gap:10}}>
                  <div style={{flex:1}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:8}}>
                      <span style={{fontFamily:"Georgia,serif",fontSize:13,color:C.espresso}}>
                        👤 {order.customer}
                      </span>
              ...
              {isExp&&(
                <div style={{borderTop:`1px solid ${C.parchment}`,padding:"13px 14px",background:C.cream}}>
                  {order.note&&<div style={{fontSize:12,color:C.rust,fontFamily:"Georgia,serif",marginBottom:8,fontStyle:"italic",background:"#fff8f0",padding:"8px 12px",borderRadius:8}}>📝 {order.note}</div>}
...
function WaNumberModal({message, onClose}) {
  ...
  return (
    <div>
      ...
      <div style={{fontFamily:"Georgia,serif",fontSize:13,color:C.espresso,marginBottom:14}}>Número do destinatário:</div>
...
function FinanceiroPage({orders, caixaHistory, onCaixaClose, drivers}) {
  ...
  const driverStats = drivers.map(d=>{
    const dOrders = po.filter(o=>o.driverId===d.uid||o.driverName===d.name);
    const dinheiro = dOrders.filter(o=>o.payment==="dinheiro").reduce((s,o)=>s+o.total,0);
    const fretes   = dOrders.reduce((s,o)=>s+(o.freight||0),0);
    return {...d, deliveries:dOrders.length, fretes, dinheiro, orders:dOrders};
  ...
  function buildDriverReport(d) {
    return [
      ...
      ``,`✅ Para acerto: ${fmt(d.dinheiro - d.fretes)}`,
    ].join("\n");
  }
  ...
  return (
    <div style={{padding:"12px 14px",maxWidth:640,margin:"0 auto",paddingBottom:40}}>
      ...
      {totalFrete>0&&<div style={{background:"#e0f2fe",borderRadius:12,padding:"10px 14px",marginBottom:12,fontFamily:"Georgia,serif",fontSize:13,color:"#0369a1"}}>🛵 Total em fretes: <strong>{fmt(totalFrete)}</strong></div>}
      ...
      {period==="hoje"&&(
        <Card style={{marginBottom:12,padding:14}}>
          ...
          {showCaixa&&<div style={{display:"flex",gap:8,marginTop:10}}>
          ...
          {driverStats.map(d=>(
            <div key={d.uid} style={{padding:"10px 0",borderBottom:`1px solid ${C.parchment}`}}>
              ...
              <div style={{display:"flex",gap:12,fontFamily:"Georgia,serif",fontSize:12}}>
                ...
                <span style={{color:C.sage}}>💰 Fretes: {fmt(d.fretes)}</span>
...
export default function AdminLayout({user}) {
  ...
  useEffect(()=>{
    const unsubs=[
      ...
      onSnapshot(collection(db,"users"),  snap=>setDrivers(snap.docs.map(d=>d.data()).filter(u=>u.role==="driver"))),
    ];
  ...
  async function handleSave(order) {
    ...
    const ex=clients.find(c=>c.name?.toLowerCase()===order.customer?.toLowerCase());
    if(ex?._docId) await updateDoc(doc(db,"clients",ex._docId),{orders:(ex.orders||0)+1,total:(ex.total||0)+order.total,lastOrder:order.createdAt});
    else await addDoc(collection(db,"clients"),{name:order.customer,whatsapp:order.customerPhone||"",address:order.address||"",orders:1,total:order.total,lastOrder:order.createdAt});
  ...
  function buildOrderMsg(order) {
    ...
    return [
      ...
      `👤 ${order.customer}`,order.address?`📍 ${order.address}`:"",
      ``,`*Itens:*`,...order.items.map(it=>`▪ ${it.qty}x ${it.name} — ${fmt(it.price*it.qty)}`),``,
      `💰 *Total: ${fmt(order.total)}*`,order.freight>0?`🛵 Frete: ${fmt(order.freight)}`:"",
      `💳 ${payLabel}`,order.note?`📝 ${order.note}`:"",
    ].filter(l=>l!=="").join("\n");
  }
  ...
  async function handleStatus(id,next) {
    ...
    showToast(`${ORDER_STATUS[next]?.emoji} ${ORDER_STATUS[next]?.label}`);
  }
  ...
  return (
    <div style={{minHeight:"100vh",background:C.cream}}>
      ...
      {loading ? <Spinner/> : <>
        {tab==="pedidos"  && <PedidosPage    orders={orders} onStatus={handleStatus} onDelete={handleDelete} clients={clients}/>}
        ...
        {tab==="fin"      && <FinanceiroPage orders={orders} caixaHistory={caixaHistory} onCaixaClose={handleCaixaClose} drivers={drivers}/>}
