  ...
  return (
    <div style={{padding:"12px 14px",maxWidth:640,margin:"0 auto",paddingBottom:40}}>
      ...
      <div style={{display:"flex",gap:8,marginBottom:14}}>
        ...
        <button onClick={()=>setModal({mode:"new"})} style={{padding:"11px 16px",borderRadius:12,border:"none",cursor:"pointer",background:C.terracotta,color:"#fff",fontFamily:"Georgia,serif",fontSize:13,fontWeight:700,whiteSpace:"nowrap",boxShadow:`0 3px 12px rgba(196,101,58,.3)`}}>+ Novo</button>
      </div>
          ...
          <Card key={c._docId||c.name} style={{padding:"12px 14px"}}>
            <div style={{display:"flex",alignItems:"center",gap:12}}>
              ...
              <div style={{display:"flex",gap:6,flexShrink:0}}>
                {c.whatsapp&&<WaButton phone={c.whatsapp} size={36}/>}
                <button onClick={()=>setViewCli(c)} style={{width:36,height:36,borderRadius:"50%",border:`1.5px solid ${C.parchment}`,background:C.warmWhite,color:C.brown,cursor:"pointer",fontSize:14}}>👁</button>
                <button onClick={()=>setModal({mode:"edit",client:c})} style={{width:36,height:36,borderRadius:"50%",border:`1.5px solid ${C.parchment}`,background:C.warmWhite,color:C.brown,cursor:"pointer",fontSize:14}}>✏️</button>
              </div>
            </div>
          </Card>
        ))}
      ...
      {viewCli&&(
        <Modal onClose={()=>setViewCli(null)}>
          ...
          <div style={{fontFamily:"Georgia,serif",fontSize:12,fontWeight:700,color:C.rust,letterSpacing:1,marginBottom:8}}>ÚLTIMOS PEDIDOS</div>
